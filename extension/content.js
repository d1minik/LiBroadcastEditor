...
if (!shared) {
  throw new Error('LiBroadcastShared was not loaded before content.js.');
}
...
function applySettings(settings) {
  ...
  const liveboardScale = clamp(
    parseNumber(settings.liveboardScale, parseNumber(defaultSettings.liveboardScale, 82)),
    55,
    140
  ) / 100;
  const liveboardFlagScale = clamp(
    parseNumber(settings.liveboardFlagScale, parseNumber(defaultSettings.liveboardFlagScale, 100)),
    60,
    160
  ) / 100;
  ...
  const liveboardTitleOpacity = parseOpacity(
    settings.liveboardTitleOpacity,
    parseNumber(defaultSettings.liveboardTitleOpacity, 100)
  );
  ...
  setOrRemoveProperty(
    '--bc-liveboard-name-font-weight',
    settings.useCustomLiveboardStyle ? settings.liveboardNameFontWeight || '' : ''
  );
  setOrRemoveProperty(
    '--bc-liveboard-title-color',
    settings.useCustomLiveboardStyle
      ? colorWithOpacity(
  ...
  setOrRemoveProperty(
    '--bc-liveboard-clock-font',
    settings.useCustomLiveboardStyle ? resolvedLiveboardClockFont : ''
  );
  setOrRemoveProperty(
    '--bc-liveboard-clock-font-weight',
    settings.useCustomLiveboardStyle ? settings.liveboardClockFontWeight || '' : ''
  );
...
function reportBoundaryError(boundary, error) {
  console.error(`[LiBroadcastEditor] ${boundary}`, error);
}

function getRuntimeError(fallbackMessage) {
...
function loadInitialSettings() {
  chrome.storage.sync.get({ ...defaultSettings }, (settings) => {
    ...
    try {
      ...
      reportBoundaryError('initial-settings-apply', error);
    }
  });
}
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action !== 'updateStyles') return undefined;

  try {
    applySettings(request.settings);
    sendResponse({ ok: true });
  } catch (error) {
    reportBoundaryError('runtime.onMessage', error);
